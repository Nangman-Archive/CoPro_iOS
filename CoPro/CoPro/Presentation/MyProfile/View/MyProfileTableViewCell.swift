...
extension MyProfileTableViewCell {
   ...
   func configureButton(at index: Int) {
      ...
      case 6:
         greaterthanButton.addTarget(self, action: #selector(didTapBlockedUserListButton), for: .touchUpInside)
   ...
   // 차단 유저 조회
   @objc func didTapBlockedUserListButton(_ sender: UIButton) {
      delegate?.didTapBlockedUserListButton(in: self)
   }
}
