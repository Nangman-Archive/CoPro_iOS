...
extension MyProfileAPI {
   ...
   /// MARK: - 차단 유저 목록 조회
   public func getBlockedUserList(token: String, completion: @escaping(NetworkResult<Any>) -> Void) {
      AFManager.request(MyProfileRouter.getBlockedList(token: token)).responseData { response in
         if let statusCode = response.response?.statusCode {
            if statusCode == 401 {
               // 토큰 재요청 함수 호출
               LoginAPI.shared.refreshAccessToken { result in
                  ...
                  case .success(let loginDTO):
                     print("토큰 재발급 성공: \(loginDTO)")
                     DispatchQueue.main.async {
                        self.getBlockedUserList(token: loginDTO.data.accessToken, completion: completion)
                     }
                  case .failure(let error):
                     print("토큰 재발급 실패: \(error)")
                  }
               ...
               // 상태 코드가 401이 아닌 경우, 결과를 컴플리션 핸들러로 전달
               self.disposeNetwork(response, dataModel: BlockedDTO.self, completion: completion)
            }
         } else {
            // 상태 코드를 가져오는데 실패한 경우, 결과를 컴플리션 핸들러로 전달
            self.disposeNetwork(response, dataModel: BlockedDTO.self, completion: completion)
